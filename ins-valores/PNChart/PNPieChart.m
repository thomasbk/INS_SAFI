...
-(id)initWithFrame:(CGRect)frame items:(NSArray *)items Tipo:(NSString *) tipo{
...
- (void)didTouchAt:(CGPoint)touchLocation
{
    ...
    if ([self.delegate respondsToSelector:@selector(userClickedOnPieIndexItem:Tipo:Data:)]) {
        [self.delegate userClickedOnPieIndexItem:index Tipo:self.tipoGrafico Data:self.items[index]];
    }
...
- (UIView*) getLegendWithMaxWidth:(CGFloat)mWidth{
   ...
   /* CGFloat singleRowHeight = [PNLineChart sizeOfString:@"Test"
    ...
    for (PNPieChartDataItem *pdata in self.items) {
        /* Expected label size*/
        NSString *descripcionLegend = [NSString stringWithFormat:@"%@ - %.f%%",pdata.textDescription,pdata.value];
        CGSize labelsize = [PNLineChart sizeOfString:descripcionLegend
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(15, (30-labelsize.height)/2, labelsize.width, labelsize.height)];
        label.text = descripcionLegend;
        ...
        UIView *viewLabel = [[UIView alloc] initWithFrame:CGRectMake(x + beforeLabel, y, 250, fmaxf(30, labelsize.height+3))];
        viewLabel.backgroundColor = pdata.color;
        
        rowWidth += labelsize.width + beforeLabel;
        totalWidth = self.legendStyle == PNLegendItemStyleSerial ? fmaxf(rowWidth, totalWidth) : fmaxf(totalWidth, viewLabel.frame.size.width + beforeLabel);
        
        viewLabel.tag = counter;
        
        //The setup code (in viewDidLoad in your view controller)
        UITapGestureRecognizer *singleFingerTap = [[UITapGestureRecognizer alloc] initWithTarget:self
                                                                                          action:@selector(clicOpcionLegend:)];
        [viewLabel addGestureRecognizer:singleFingerTap];
        
        rowMaxHeight = fmaxf(rowMaxHeight, viewLabel.frame.size.height+5);
        x += self.legendStyle == PNLegendItemStyleStacked ? 0 : viewLabel.frame.size.width + beforeLabel;
        y += self.legendStyle == PNLegendItemStyleStacked ? viewLabel.frame.size.height + 5 : 0;
        
        totalHeight = self.legendStyle == PNLegendItemStyleSerial ? fmaxf(totalHeight, rowMaxHeight + y) : totalHeight + viewLabel.frame.size.height + 5;
                
        [Functions redondearView:viewLabel Color:@"FFFFFF" Borde:0 Radius:15];
                
        // Imagen indicando profundidad
        if([pdata.profundidad isEqualToString:@"true"]){
            UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(viewLabel.frame.size.width-20, (viewLabel.frame.size.height-17)/2, 10, 17)];
            [imageView setImage:[UIImage imageNamed:@"flecha_blanca_p"]];
            [viewLabel addSubview:imageView];
        }
        
        
        [viewLabel addSubview:label];
        
        [legendViews addObject:viewLabel];
        
        counter ++;
    }
    
    UIView *legend = [[UIView alloc] initWithFrame:CGRectMake(0, 0, totalWidth, totalHeight)];
    [legend setUserInteractionEnabled:YES];
...
- (void)clicOpcionLegend:(UITapGestureRecognizer *)recognizer
{
    if ([self.delegate respondsToSelector:@selector(userClickedOnPieIndexItem:Tipo:Data:)]) {
        [self.delegate userClickedOnPieIndexItem:recognizer.view.tag Tipo:self.tipoGrafico Data:self.items[recognizer.view.tag]];
    }
}
