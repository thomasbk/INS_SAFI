...
@interface BEMSimpleLineGraphView () {
    ...
    /// All of the X-Axis Label Points Vertical Line
    NSMutableArray *xAxisLabelPointsVertical;
...
- (void)commonInit {
    ...
    xAxisLabelPointsVertical = [NSMutableArray array];
...
#pragma clang diagnostic pop
    ...
    if (numberOfPoints == 0) {
        ...
        self.noDataLabel.text = noDataText ?: NSLocalizedString(@"Sin datos", nil);
#else
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        if (self.enablePopUpReport == YES && self.alwaysDisplayPopUpLabels == NO) {
            if ([self.delegate respondsToSelector:@selector(popUpViewForLineGraph:)]) {
                ...
                NSString * maxValueString =  [formatter stringFromNumber:[self calculateMaximumPointValue]];
                NSString *minValueString = [formatter stringFromNumber:[self calculateMinimumPointValue]];
...
- (void)drawEntireGraph {
    ...
    if (self.enableYAxisLabel) {
        ...
        if (self.autoScaleYAxis == YES){
            // Modificación para que muestra visualmente las comas
            NSNumberFormatter * formatter = [NSNumberFormatter new];
            [formatter setNumberStyle:NSNumberFormatterDecimalStyle];
            [formatter setMaximumFractionDigits:2]; // Set this if you need 2 digits
            [formatter setGroupingSeparator:@","];
            [formatter setDecimalSeparator:@"."];
            [formatter setUsesGroupingSeparator:YES];
            NSString * maxValueString =  [formatter stringFromNumber:[NSNumber numberWithFloat:self.maxValue]];
            NSString *minValueString = [formatter stringFromNumber:[NSNumber numberWithFloat:self.minValue]];
...
- (void)drawLine {
    ...
    if (self.enableReferenceXAxisLines || self.enableReferenceYAxisLines) {
        ...
        if (self.verticalLine == YES) {
            line.enableRefrenceLines = YES;
            line.arrayOfVerticalRefrenceLinePoints = xAxisLabelPointsVertical;
        }
...
#pragma clang diagnostic pop
        ...
        if (numberOfGaps >= (numberOfPoints - 1)) {
            ...
            @autoreleasepool {
                ...
                for (int i = 1; i <= (numberOfPoints/numberOfGaps); i++) {
                    ...
                    if (self.verticalLine == YES && (self.indexVerticalLine<index || i==(numberOfPoints/numberOfGaps)) && prevVerticalLine==-1) {
                        // Restamos 2 xq i comienza en 1 y xq el previo esta en el elemento anterior
                    
                        prevVerticalLine = i - 2;
                        if(prevVerticalLine<0){
                            prevVerticalLine = 0;
                        }
                        [self putVerticalLine];
                    }
    ...
    [xAxisLabels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL *stop) {
        ...
        if (self.verticalLine){
            // Revisamos si la línea vertical a pintar se overpone a otro elemento
            NSString *xAxisLabelTextVertical = [self xAxisTextForIndex:self.indexVerticalLine];
            UILabel *labelXAxisVertical = [self xAxisLabelWithText:xAxisLabelTextVertical atIndex:self.indexVerticalLine];
            CGRect rPrev = CGRectIntersection(label.frame, labelXAxisVertical.frame);
            if (!CGRectIsNull(rPrev)){
                [overlapLabels addObject:label];
            }
...
- (void) putVerticalLine{
    [xAxisLabelPointsVertical removeAllObjects];
    
    NSString *xAxisLabelText = [self xAxisTextForIndex:self.indexVerticalLine];
    
    UILabel *labelXAxis = [self xAxisLabelWithText:xAxisLabelText atIndex:self.indexVerticalLine];
    
    NSNumber *xAxisLabelCoordinate;
    if (self.positionYAxisRight) {
        xAxisLabelCoordinate = [NSNumber numberWithFloat:labelXAxis.center.x];
        [xAxisLabelPointsVertical addObject:xAxisLabelCoordinate];
    } else {
        xAxisLabelCoordinate = [NSNumber numberWithFloat:labelXAxis.center.x - self.YAxisLabelXOffset];
        [xAxisLabelPointsVertical addObject:xAxisLabelCoordinate];
    }
...
- (void)drawYAxis {
    ...
    if (self.autoScaleYAxis) {
        ...
        for (NSNumber *dotValue in dotValues) {
            ...
            NSNumberFormatter * formatter = [NSNumberFormatter new];
            [formatter setNumberStyle:NSNumberFormatterDecimalStyle];
            [formatter setMaximumFractionDigits:2]; // 2 digits
            [formatter setGroupingSeparator:@","];
            [formatter setDecimalSeparator:@"."];
            [formatter setUsesGroupingSeparator:YES];
            NSString * valueString =  [formatter stringFromNumber:dotValue];
            labelYAxis.text = valueString;
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint {
    ...
    if (!self.usingCustomPopupView) {
        ...
        self.popUpLabel.text = valueString;
        self.popUpLabel.center = self.popUpView.center;
    }
}
