...
static NSString * const JMLetterMaskString = @"L";
static NSString * const JMNumberMaskString = @"0";
static NSString * const JMNumberMaskAlphanumeric = @"A";
...
- (NSString *)maskString:(NSString *)string
{
    ...
    for (int i = 0; i < string.length; i++) {
        ...
        if ([currentCharacter isEqualToString:maskCharacter]) {
            ...
            while (![maskCharacter isEqualToString:JMLetterMaskString] && ![maskCharacter isEqualToString:JMNumberMaskString] && ![maskCharacter isEqualToString:JMNumberMaskAlphanumeric]) {
            ...
            BOOL isValidAlphanumeric = [self isValidLetterString:currentCharacter] || [self isValidNumberString:currentCharacter];
            
            if (isValidLetter || isValidNumber) {
                [formattedString appendString:currentCharacter];
            } else if([maskCharacter isEqualToString:JMNumberMaskAlphanumeric] && isValidAlphanumeric){
                [formattedString appendString:currentCharacter];
            } else {
                return nil;
            }
