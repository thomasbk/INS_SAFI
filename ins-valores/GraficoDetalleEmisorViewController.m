//
//  GraficoDetalleEmisorViewController.m
//  INSValores
//
...
@interface GraficoDetalleEmisorViewController ()

@end

@implementation GraficoDetalleEmisorViewController
...
- (void)viewDidLoad {
    ...
    self.tituloPantalla.text = self.titulo;
...
- (void) viewWillAppear:(BOOL)animated{
    ...
    [tracker set:@"&cd" value:[NSString stringWithFormat:@"GRÁFICO DETALLE COMPOSICIÓN POR EMISOR"]];
...
// Llnamos el scroll view con el gráfico
...
- (void)hydrateDatasets:(int) meses {
    ...
    if (networkStatus == ReachableViaWiFi || networkStatus == ReachableViaWWAN){
        ...
        [self presentViewController:alert animated:YES completion:^{
            ...
            NSDictionary *params = [[NSDictionary alloc] initWithObjectsAndKeys:self.idCuenta, @"CU", self.idPortafolio, @"SC", self.numOperacion, @"NU", self.codIsin, @"CI", comandoMes, @"PE", fDesde, @"FD", fHasta, @"FH", usrTok, @"TK", nil];
        ...
        // Mostramos el error
        [self showAlert:self.titulo withMessage:@"No hay conexión a Internet" withClose:false];
    }
}
...
- (void)requestFinished:(ASIHTTPRequest *)request {
    ...
    if(data != nil){
        ...
        if ([cod isEqualToString:@"0"])
        {
            ...
            // Mostramos el error
            [self showAlert:self.titulo withMessage:[[dataString objectForKey:@"Respuesta"] objectForKey:@"Mensajes"] withClose:true];
        } else {
            // Limpiamos los datos
            [self.arrayOfValues removeAllObjects];
            [self.arrayOfDates removeAllObjects];
            ...
            // Mostramos el error
            [self showAlert:self.titulo withMessage:[[dataString objectForKey:@"Respuesta"] objectForKey:@"Mensajes"] withReturn:false];
        }
    } else {
        // Limpiamos los datos
        [self.arrayOfValues removeAllObjects];
        ...
        // Cerramos el alert de loading
        [self closeAlertLoading];
        [self showAlert:self.titulo withMessage:@"Ha ocurrido un error con la solicitud" withReturn:true];
    }
}
